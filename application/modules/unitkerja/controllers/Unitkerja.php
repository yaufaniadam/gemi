...
class Unitkerja extends MY_Controller
{
	...
	public function detail_unit($kantor = 0, $id = 0, $kinerja = 0)
	{
	...
	public function add_ce()
	{
		if ($this->input->post('submit')) {
			$data = array(
				'id_kantor' => get_kantor_by_user()['id_kantor'],
				'maqasid' => $this->input->post('maqasid'),
				'sub_maqasid' => $this->input->post('sub_maqasid'),
				'tanggal' => $this->input->post('tanggal'),
				'wakif' => $this->input->post('wakif'),
				'benda_wakaf' => $this->input->post('wakif'),
				'user_id' => $this->session->userdata('benda_wakaf'),
				'nama_program' => $this->input->post('nama_program'),
				'jumlah_peserta' => $this->input->post('jumlah_peserta'),
				'terikat' => $this->input->post('terikat'),
			...
			$data = $this->security->xss_clean($data);
			$result = $this->unitkerja_model->add_ce($data);
			if ($result) {
				$this->session->set_flashdata('msg', 'Data telah disimpan!');
				redirect(base_url('unitkerja/index/' . $this->input->post('sub_maqasid')));
			}
		} else {
			$data['kantor'] = $this->unitkerja_model->get_kantor_by_id(get_kantor_by_user()['id_kantor']);
			$data['unit'] = $this->unitkerja_model->get_unit_by_id(get_kantor_by_user()['id_unit']);
			$data['view'] = 'detail_unit';
			$this->load->view('admin/layout', $data);
		}
	}
	...
	public function hapus_entry($submaqasid = 0, $id = 0)
	{
		$unit = get_kode_unit_kerja_by_id(get_kantor_by_user()['id_unit']);
		$this->db->delete('ci_' . $unit, array('id' => $id));
		$this->session->set_flashdata('msg', 'Data berhasil dihapus!');
		redirect(base_url('unitkerja/index/' . $submaqasid));
	}
}
