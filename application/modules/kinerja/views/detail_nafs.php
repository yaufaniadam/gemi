			...
			if (count($nafs) > 0) {
				...
				foreach ($recNew as $key => $values) // For every field name (id, name, last_name, gender)
				{
					if ($i == 1) {
						...
						foreach ($values as $cell) // for every sub-array iterate through all values
						{
							echo "\t<th class='text-center'><a title='Hapus' class='delete btn btn-xs btn-danger' data-toggle='modal' data-target='#confirm-delete' data-href='" . base_url('kinerja/individu/hapus_nafs/' . $cell) . "'> <i class='fa fa-trash-o'></i></a> <a title='Edit' class='btn btn-xs btn-info' href='" . base_url('kinerja/individu/nafs/' . $cell) . "'> <i class='fa fa-pencil'></i></a></th>\n"; // write cells next to each other
						}
						echo "</tr>\n"; // end row
