<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    function get_kantor_by_user() {
        ...
        $penempatan = $CI->db->query('SELECT * FROM ci_penempatan where user_id='.$id);

        if($penempatan->num_rows() > 0) {            
            $kantor = $penempatan->row();
            $id_unit =  $kantor->id_unit;
            $query = $CI->db->query('SELECT * FROM ci_kantor where id='. $kantor->id_kantor);
            $row = $query->result_array();
            return array($row,$id_unit);
        } else {
            return $penempatan_exist = 0;
        }
